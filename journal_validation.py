...
def main():
	...
	#key, title, date, author_rank
	x = data[['사번', '논문제목', '게재년월', '저자역할']]

	#reading professor data
	prof = pd.read_excel('prof.xlsx')

	
	#'''
	#1st phase: validating and writing into csv file using scopus data----------------------------------------------------------

	#read extracted schopus data here
	scopus = pd.read_csv('temp/scopus_전임_v2.csv')
	s = scopus[['Title', 'cor_A', 'Author', 'vol_num', 'Date']]

	for ix in range(0, 2746):
		...
		key = x.iloc[ix]['사번']
		original_author = prof[prof["사번"]==key]
		...
		#validating the title of the paper
		if difflib.SequenceMatcher(None, x.iloc[ix]['논문제목'].lower(), title.lower()).ratio()>0.60:
			x.loc[ix, 'val_title'] = 'valid'
			#x.to_csv('paper.csv', index=False)
		else:
			th = "{:.2f}".format(difflib.SequenceMatcher(None, x.iloc[ix]['논문제목'].lower(), title.lower()).ratio())
			x.loc[ix, 'val_title'] = 'invalid ('+str(th)+')'

			

		
		#validating the rank(role) of the author
		#first author
		if x.iloc[ix]['저자역할'] == '제1저자':
		...
		elif x.iloc[ix]['저자역할'] == 'corresponding author':
		...
		x.to_csv('scopus_valid.csv', encoding='utf-8-sig', index=False)
	...
	x2 = data[['사번', '논문제목', '게재년월', '저자역할']]

	#read WoS data here
	wos_data = pd.read_csv('temp/wos_data_전임_v2.csv')
	...
	for ix in range(0, 2746):
		...
		if wos.iloc[ix]['Title'] == '0':
			print('skipping')
			x2.loc[ix, 'val_title'] = 'unknown--'
			#x.loc[ix, 'val_pages'] = 'unknown--'
			x2.loc[ix, 'val_rank'] = 'unknown--'
			#x2.to_csv('wos_valid.csv', index=False)
			continue
		...
		#validating the title of the paper
		if difflib.SequenceMatcher(None, x2.iloc[ix]['논문제목'].lower(), title.lower()).ratio()>0.60:
			x2.loc[ix, 'val_title'] = 'valid'
			#x.to_csv('paper.csv', index=False)
		else:
			th = "{:.2f}".format(difflib.SequenceMatcher(None, x2.iloc[ix]['논문제목'].lower(), title.lower()).ratio())
			x2.loc[ix, 'val_title'] = 'invalid ('+str(th)+')'

		
		#validating the rank(role) of the author
		#first author
		if x2.iloc[ix]['저자역할'] == '제1저자':
			if compare_name(korean, author_list[0]) or compare_name(english, author_list[0]):
		...
		#corresponding author
		elif x2.iloc[ix]['저자역할'] == 'corresponding author':
		...
		x2.to_csv('wos_valid.csv', encoding='utf-8-sig', index=False)
	...
	#3rd phase: increase hitrate using KCI data-------------------------------------------------------------------------------------
	print('\nI am performing the 3rd phase----\n')
	x2 = data[['사번', '논문제목', '게재년월', '저자역할']]

	#read KCI data here
	kci_data = pd.read_csv('temp/kci_전임_v2.csv')
	kci = kci_data[['Title', 'cor_A', 'Author', 'vol_num']]

	for ix in range(0, 2746):
		print("Now doing row number: ", ix)

		if kci.iloc[ix]['Title'] == '0':
			print('skipping')
			x2.loc[ix, 'val_title'] = 'unknown--'
			#x.loc[ix, 'val_pages'] = 'unknown--'
			x2.loc[ix, 'val_rank'] = 'unknown--'
		...
		title = kci.iloc[ix]['Title']
		if title.find("\n") != -1:
			ind = title.find("\n")
			title = title[0: ind : ]
		author = kci.iloc[ix]['Author']
		if author.find(", "):
			author_list = author.split(", ")
		else:
			author_list = [author]
		cor_a = kci.iloc[ix]['cor_A']
		if cor_a.find(", "):
			cor_a = cor_a.split(", ")
		else:
			cor_a = [cor_a]
		...
		if x2.iloc[ix]['저자역할'] == '제1저자':
			...
			if author1.find("/") != -1:
				ind = author1.find("/")
				kor = author1[0: ind-1 : ]
				eng = author1[ind+1 : len(author1) : ]
			else:
				kor = author1
				eng = author1
			if compare_name(korean, kor) or compare_name(english, eng):
				x2.loc[ix, 'val_rank'] = 'valid'
			else:
				x2.loc[ix, 'val_rank'] = 'invalid'
		...
		x2.to_csv('kci_valid.csv', encoding='utf-8-sig', index=False)	
	...
	#merging phase: merging all 3 data-------------------------------------------------------------------------------------
	print('\nI am performing the merging phase----\n')

	data2 = pd.read_csv('scopus_valid.csv')
	x2 = data2[['사번', '논문제목', '게재년월', '저자역할', 'val_title', 'val_rank']]

	wos_data = pd.read_csv('wos_valid.csv')
	wos = wos_data[['사번', '논문제목', '게재년월', '저자역할', 'val_title', 'val_rank']]

	kci_data = pd.read_csv('kci_valid.csv')
	kci = kci_data[['사번', '논문제목', '게재년월', '저자역할', 'val_title', 'val_rank']]

	for ix in range(0, 2746):
		print("Now doing row number: ", ix)

		if wos.iloc[ix]['val_title'] == 'valid':
			x2.loc[ix, 'val_title'] = 'valid'
			
		if wos.iloc[ix]['val_rank'] == 'valid':
			x2.loc[ix, 'val_rank'] = 'valid'

		x2.to_csv('journal_paper.csv', encoding = 'utf-8-sig', index=False)

	for ix in range(0, 2746):
		print("Now doing row number: ", ix)

		if kci.iloc[ix]['val_title'] == 'valid':
			x2.loc[ix, 'val_title'] = 'valid'
			
		if kci.iloc[ix]['val_rank'] == 'valid':
			x2.loc[ix, 'val_rank'] = 'valid'
	...
	#4th phase: increase hitrate using Google Scholar data-------------------------------------------------------------------------------------

	data2 = pd.read_csv('journal_paper.csv')
	x2 = data2[['사번', '논문제목', '게재년월', '저자역할', 'val_title', 'val_rank']]
	cnt = 0

	for ix in range(363, 2746):
		sleep_time = random.randint(7,10)
		print("Now doing row number: ", ix)

		if x2.iloc[ix]['val_title'] == 'unknown--' and cnt < 10:
			cnt = cnt+1
			print('\n--------------I found unknown data at ', ix, "My counter is: ", cnt)
			key = x.iloc[ix]['사번']
			original_author = prof[prof["사번"]==key]
			korean = original_author['국문'].iloc[0]
			english = original_author['영문'].iloc[0]

			all_authors, vancouver_authors, title, pages = google_scholar_selenium(x.iloc[ix]['논문제목'])
			author_list = vancouver_authors.split(', ')
			...
			#corresponding author
				...
				x2.loc[ix, 'val_rank'] = 'cor_unknown'

			else:
				x2.loc[ix, 'val_rank'] = 'unknown++'	

			x2.to_csv('journal_paper.csv', index=False)	
			time.sleep(sleep_time)

		elif x2.iloc[ix]['val_title'] == 'unknown--' and cnt >= 10:
			print("Unknown found but already has 10...")	
	...
	'''
	#5th phase: semi-automated validation using extracted raw data -----------------------------------------------------------------------------

	data2 = pd.read_csv('final_results.csv')
	x2 = data2[['사번', '논문제목', '게재년월', '저자역할', 'val_title', 'val_rank']]
	print('\n\n', x2.shape)
	unk = x2[x2['val_title']=='unknown--']
	#print(unk.shape)
	#print(unk.iloc[19]['논문제목'])

	data3 = pd.read_csv('final.csv')
	...
	for ix in range(0, 2746):
		...
		if x2.iloc[ix]['val_title'] == 'unknown--' and cnt < 128:
			cnt = cnt+1
			print('\n--------------I found unknown data at ', ix, "My counter is: ", cnt)
			key = x.iloc[ix]['사번']
			original_author = prof[prof["사번"]==key]
			...
			Vancouver = data3.iloc[cnt]['Title']

			period = Vancouver.find('.')
			vancouver_authors = Vancouver[0 : period : ]

			index = Vancouver.find('TeV.')
			all_authors = Vancouver[0 : index-2 : ]
			index2 = Vancouver.find('"', index+2)
			title = Vancouver[index+1 : index2-1 : ]
			index = Vancouver.find('):')
			if index != -1:
				pages = Vancouver[index+3 : len(Vancouver)-1 : ]
			else:
				pages = 'unknown'
