...
def main():
	...
	x = data[['사번', '특허연번', '특허방법', '특허권자구분', '특허명', '취득국가']]
	
	#read file with author validation (because it was seperate from patent data)
	c = pd.read_excel('patent_aff.xlsx')

	#read extracted data for validation
	s = pd.read_csv('patent_val_v1_h.csv')
	#'''
	for ix in range(0, 347):
		print("Now doing row number: ", ix)


		#ins 1: sorting
		#if not published yet
		if x.iloc[ix]['특허방법'] == '출원,공개,공고':
			print('none')
			x.loc[ix, 'val_title'] = 'none'
			x.loc[ix, 'val_특허권자구분'] = 'none'
			x.loc[ix, 'val_특허방법'] = 'none'
			x.loc[ix, 'val_취득국가'] = 'none'
			
			continue

		#if not found on the web	
		elif s.iloc[ix]['특허명'] == '0':
			print('unknown')
			x.loc[ix, 'val_title'] = 'unknown'
			x.loc[ix, 'val_특허권자구분'] = 'unknown'
			x.loc[ix, 'val_특허방법'] = 'unknown'
			x.loc[ix, 'val_취득국가'] = 'unknown'
		...
		or_title = x.iloc[ix]['특허명']
		or_title = or_title[0 : or_title.find('(') - 1 : ]


		
		#ins 2: validating the title of the patent
		if difflib.SequenceMatcher(None, or_title.lower(), title.lower()).ratio()>0.90:
			x.loc[ix, 'val_title'] = 'valid'
			#x.to_csv('paper.csv', index=False)
		else:
			th = "{:.2f}".format(difflib.SequenceMatcher(None, x.iloc[ix]['특허명'].lower(), title.lower()).ratio())
			x.loc[ix, 'val_title'] = 'invalid ('+str(th)+')'

			
		#ins 3: validating 특허권자구분 column (inha or not)
		if x.iloc[ix]['특허권자구분'] == '인하대학교':
			if s.iloc[ix]['특허권자구분'] == '인하대학교 산학협력단':
				x.loc[ix, 'val_특허권자구분'] = 'valid'
			else:
				x.loc[ix, 'val_특허권자구분'] = 'invalid'
		else:
			x.loc[ix, 'val_특허권자구분'] = 'valid'

		
		#ins 4: checking if 등록 (enrolled or not)   	
		if '등록' in s.iloc[ix]['특허방법']:
			x.loc[ix, 'val_특허방법'] = 'valid'
		else:
			x.loc[ix, 'val_특허방법'] = 'invalid'


		#ins 5: checking if 국내 (domestic or not)
		if x.iloc[ix]['취득국가'] == '한국':
			if "국내" in s.iloc[ix]['취득국가']:
				x.loc[ix, 'val_취득국가'] = 'valid'
			else:
				x.loc[ix, 'val_취득국가'] = 'invalid'
		else:
			x.loc[ix, 'val_취득국가'] = 'valid'


		x.to_csv('patent_validation.csv', encoding = 'utf-8-sig', index=False)

	print('\n\nDone------------\n\n')	
	#'''


	#Ins 6: validating prof name 
	#'''
	x2 = pd.read_csv('patent_validation.csv')
	jn = 0
	for iy in range(0, 1194):
		...
		if c.iloc[iy]['교수본인'] == 'Y':
			...
			#print(or_author)
			sss = s[(s["사번"]==c_id) & (s["특허연번"]==c_num)]

			if or_author in sss['발명자이름'].iloc[0]:
				x2.loc[jn, 'val_main_author'] = 'valid '
			elif sss['발명자이름'].iloc[0] == '0':
				x2.loc[jn, 'val_main_author'] = 'unknown '
			else: 
				x2.loc[jn, 'val_main_author'] = 'invalid '

			jn += 1


		x2.to_csv('patent_validation.csv', encoding='utf-8-sig', index=False)		
