...
def main():
	...
	data = pd.read_excel('old_format.xlsx')
	print(data.values.shape)
	#key, title, author_count, author_rank, pages
	x = data[['키', '논문제목', '총저자수', '저자역할', '게재면']]
	print(x.shape)
	...
	scopus = pd.read_csv('temp/scopus_v2.csv')
	s = scopus[['Title', 'cor_A', 'Author', 'vol_num']]

	for ix in range(0, 706):
		...
		if difflib.SequenceMatcher(None, x.iloc[ix]['논문제목'].lower(), title.lower()).ratio()>0.90:
		...
		#validating the pages of the paper
		if compare_name(str(x.iloc[ix]['게재면']), pages):
		...
		#co-author	
		elif x.iloc[ix]['저자역할'] == '공동저자' or x.iloc[ix]['저자역할'] == '공동':
		...
		elif x.iloc[ix]['저자역할'] == '교신저자' or x.iloc[ix]['저자역할'] == '책임':
		...
		x.to_csv('recruitment_validation.csv', encoding='utf-8', index=False)
	...
	data2 = pd.read_csv('recruitment_validation.csv')
	x2 = data2[['키', '논문제목', '총저자수', '저자역할', '게재면', 'val_title', 'val_pages', 'val_rank']]

	wos_data = pd.read_csv('temp/wos_v2.csv')
	...
	for ix in range(0, 706):
		...
		if x2.iloc[ix]['val_title'] == 'unknown--' and wos.iloc[ix]['Title'] != '0':
			...
			#validating the title of the paper
			if difflib.SequenceMatcher(None, x2.iloc[ix]['논문제목'].lower(), title.lower()).ratio()>0.90:
			...
			#validating the pages of the paper
			if compare_name(str(x2.iloc[ix]['게재면']), pages):
			...
			#co-author	
			elif x2.iloc[ix]['저자역할'] == '공동저자' or x2.iloc[ix]['저자역할'] == '공동':
				if coauthor_list(korean, author_list) or coauthor_list(english, author_list):
					x2.loc[ix, 'val_rank'] = 'valid'
				else:
					x2.loc[ix, 'val_rank'] = 'invalid'
			...
			#corresponding author
			elif x2.iloc[ix]['저자역할'] == '교신저자' or x2.iloc[ix]['저자역할'] == '책임':
			...
			x2.to_csv('recruitment_validation.csv', encoding='utf-8', index=False)
	...
	kci_data = pd.read_csv('temp/kci_v2.csv')
	kci = kci_data[['Title', 'cor_A', 'Author', 'vol_num']]

	for ix in range(0, 706):
		print("Now doing row number: ", ix)

		if x2.iloc[ix]['val_title'] == 'unknown--' and kci.iloc[ix]['Title'] != '0':
			print('\n--------------I found new data at ', ix)
			...
			#validating the pages of the paper
			if compare_name(str(x2.iloc[ix]['게재면']), str(pages)):
				x2.loc[ix, 'val_pages'] = 'valid'
			else:
				x2.loc[ix, 'val_pages'] = 'invalid'	
	...
	data2 = pd.read_csv('final_results_2.csv')
	x2 = data2[['키', '논문제목', '총저자수', '저자역할', '게재면', 'val_title', 'val_pages', 'val_rank']]
	cnt = 0

	for ix in range(352, 706):
		...
		if x2.iloc[ix]['val_title'] == 'unknown--' and cnt < 10:
			...
			x2.to_csv('final_results_2.csv', index=False)	
