...
def main():
	...
	s = scopus[['사번', '논문연번', 'Affilation', 'val_title']]
	...
	for ix in range(0, 14770):
		...
		sss = s[(s["사번"]==x_id) & (s["논문연번"]==x_num)]
		#print('\nsss: ' , sss)
		s_aff = sss.iloc[0]['Affilation']
		s_val_title = sss.iloc[0]['val_title']

		if s_val_title != 'valid':
			x.loc[ix, 'affiliation'] = 'unknown--'
			print('\t\t\tunknown--')
			x.to_csv('combine_aff.csv', index=False)
			continue
		...
		if x_rank > len(s_aff):
			x.loc[ix-1, 'affiliation'] = 'unknown++'
			x.loc[ix, 'affiliation'] = 'unknown++'
			print('\t\t\tunknown++')
			x.to_csv('combine_aff.csv', index=False)
			continue
		...
		s_aff = s_aff[x_rank-1]
		if s_aff[s_aff.find('[')+1 : s_aff.find(']') : ] == 'Inha University':
			x.loc[ix, 'affiliation'] = 'inside'
			print('\t\t\tinside')
		else:
			x.loc[ix, 'affiliation'] = 'outside'
			print('\t\t\toutside')
	...
	s = wos_data[['사번', '논문연번', 'Affilation', 'val_title']]
	...
	for ix in range(0, 14770):
		...
		if s_val_title != 'valid':
			x.loc[ix, 'affiliation'] = 'unknown--'
			print('\t\t\tunknown--')
			x.to_csv('wos_aff.csv', index=False)
			continue
		...
		s_aff = s_aff[x_rank-1]
		if s_aff[s_aff.find('[')+1 : s_aff.find(']') : ] == 'Inha Univ':
			x.loc[ix, 'affiliation'] = 'inside'
			print('\t\t\tinside')
		else:
			x.loc[ix, 'affiliation'] = 'outside'
	...
	kci_data = pd.read_csv('temp/KCI_전임교원 논문.csv')
	s = kci_data[['사번', '논문연번', 'Affilation', 'val_title']]
	...
	for ix in range(0, 14770):
		...
		if s_val_title != 'valid':
			x.loc[ix, 'affiliation'] = 'unknown--'
			print('\t\t\t\tunknown--')
			x.to_csv('kci_aff.csv', index=False)
			continue
		...
		s_aff = s_aff[x_rank-1]
		if s_aff[s_aff.find('[')+1 : s_aff.find(']') : ] == '인하대학교':
			x.loc[ix, 'affiliation'] = 'inside'
			print('\t\t\t\tinside')
		else:
			x.loc[ix, 'affiliation'] = 'outside'
	...
	x2 = pd.read_csv('combine_aff.csv')

	wos = pd.read_csv('wos_aff.csv')

	kci = pd.read_csv('kci_aff.csv')

	for ix in range(0, 14770):
		print("Now doing row number: ", ix)

		if wos.iloc[ix]['affiliation'] == 'inside':
			x2.loc[ix, 'affiliation'] = 'inside'

		if wos.iloc[ix]['affiliation'] == 'outside':
			x2.loc[ix, 'affiliation'] = 'outside'	
		
		if kci.iloc[ix]['affiliation'] == 'inside':
			x2.loc[ix, 'affiliation'] = 'inside'

		if kci.iloc[ix]['affiliation'] == 'outside':
			x2.loc[ix, 'affiliation'] = 'outside'	

		x2.to_csv('combine_aff.csv', index=False)
	...
	print('\nI am performing the real validation----\n')
	...
	for ix in range(0, 14770):
		...
		if x2.iloc[ix]['소속'] == '인하대학교': 
			if x2.iloc[ix]['affiliation'] == 'inside':
				x2.loc[ix, 'val_aff'] = 'valid'
			elif x2.iloc[ix]['affiliation'] == 'outside':
				x2.loc[ix, 'val_aff'] = 'invalid'
			else:
				x2.loc[ix, 'val_aff'] = 'unknown'

		elif x2.iloc[ix]['소속'] != '인하대학교': 
			if x2.iloc[ix]['affiliation'] == 'inside':
				x2.loc[ix, 'val_aff'] = 'invalid'
			elif x2.iloc[ix]['affiliation'] == 'outside': 
				x2.loc[ix, 'val_aff'] = 'valid'
		...
		x2.to_csv('combine_aff.csv', encoding = 'utf-8-sig', index=False)
